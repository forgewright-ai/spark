...
# HOME holding a real clone at the default place (~/.spark), rc files and
# dotfiles of yours, state, config, data. Proves:
...
build_home() {
    ...
    # yours, never spark's to touch: an rc file without the spark line, a
    # .gitconfig, a tmux.conf, micro's settings -- and the other rc with
    # the marker line, the one line spark did add
    printf '# mine\n' > "$HOME/$rc"
    printf '# my tmux\n' > "$HOME/.tmux.conf"
    printf '[user]\n\tname = me\n' > "$HOME/.gitconfig"
...
build_home one
[ "$(cat "$HOME/$rc")" = "# mine" ] && ok "$rc is yours, without the spark line" || bad "setup: $rc: $(cat "$HOME/$rc" 2>&1)"
before=$(snapshot)
...
[ "$(cat "$HOME/$rc")" = "# mine" ] && [ ! -e "$HOME/$rc.bak" ] && ok "an rc file of yours is left alone (no .bak made)" || bad "$rc: $(ls -la "$HOME/$rc"* 2>&1)"
[ "$(cat "$HOME/.tmux.conf")" = "# my tmux" ] && ok ".tmux.conf is not spark's, left alone" || bad ".tmux.conf: $(cat "$HOME/.tmux.conf" 2>&1)"
...
[ -e "$HOME/.terminfo/74/tmux-256color" ] && ok "the terminfo entry is not spark's, left alone" || bad "terminfo removed"
