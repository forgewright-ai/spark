...
`XDG_*` dirs. No special hardware is needed.
...
`allowed-signers`, is a release. A user's clone follows the newest such
...
1. A model row. One row per pull request, in `models.env`: the five
...
3. A doc fix. A stale count, a broken cross-reference, a sentence that
   ...
   step by step, with two nouns, spark and spark apps;
   `tests/docs_test.py` says which words are out. The page,
   spark.forgewright.ai, is these docs rendered at the newest release,
   so a fix here reaches it with the next one. A fix in `docs/` needs
   no changelog entry.
...
only what. No trailers are required from you.
...
notice that it is missing. That is expected. Never add a real person's,
machine's or project's name to the tree regardless.
...
check --report`. It is statuses only, with no value, no path and no
name, and it runs your privacy word list over itself before printing.
