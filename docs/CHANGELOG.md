...
- A reply the cap ended says so: `! cut at the reply's length -- say:
  ...
  line), read from the server's `finish_reason`. The cap of the three
  streaming verbs (chat, explain, a bare question) is 1200 tokens now,
  `SPARK_MAX_TOKENS` in spark.env sets another (50..32000); 600 dated
  from the 1.7B days. The line's JSON answer and the editor verbs keep
  their own caps.
