...
line answers. This is the fun part: twelve small things to try, in four
acts. Each one is a habit worth keeping.
...
## Act 1: the prompt line is alive
...
## Act 2: the editor has an opinion
...
quiet for a minute and a half, and it reads the whole draft. Answer it
back with `Alt-s`.
...
in three days, then seven. The tour revises itself.
...
facts ride on every answer. They are files under your own
`~/.config/spark/`, and they go to the one server you chose: this
machine's, or the one machine of yours a client points at.
...
it answers. No lesson in this one. It is just a good show.
